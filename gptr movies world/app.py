...
<head>
    ...
    <style>
        :root { --primary: #00f2ff; --secondary: #bd00ff; --danger: #ff4757; --bg-dark: #090919; --glass: rgba(255, 255, 255, 0.05); --border: rgba(255, 255, 255, 0.1); }
        body { background-color: var(--bg-dark); background-image: radial-gradient(circle at 10% 20%, rgba(189, 0, 255, 0.2) 0%, transparent 40%), radial-gradient(circle at 90% 80%, rgba(0, 242, 255, 0.15) 0%, transparent 40%); color: white; font-family: 'Outfit', sans-serif; margin: 0; min-height: 100vh; display: flex; flex-direction: column; align-items: center; padding: 20px; }
        ...
        .download-btn { padding: 10px 20px; border-radius: 10px; border: 1px solid var(--primary); background: rgba(0, 242, 255, 0.05); color: var(--primary); font-weight: 600; cursor: pointer; transition: 0.3s; white-space: nowrap; font-size: 0.9rem; }
...
<body>
    ...
    <div class="info-wrapper">
        <div class="tips-box">
            ...
            <ul class="tips-list"><li>Check spelling (e.g. <span>Devara</span>).</li><li>If failed, search again.</li><li>Series: <span>Name S01E01</span>.</li><li>Add Year: <span>RRR 2022</span>.</li></ul>
        </div>
        <div class="notice-box">
            <div class="notice-title"><i class="fa-solid fa-triangle-exclamation"></i> Important:</div>
            <ul class="notice-list"><li>No resume support.</li><li>Stable net required.</li><li>Use IDM/FDM.</li></ul>
        </div>
    </div>
    ...
    <script>
        ...
        function generateTags(title, size) {
            ...
            if(t.includes('multi') || t.includes('dual')) tagsHtml += `<span class="tag tag-multi">MULTI AUDIO</span>`;
            ...
            if(size && size !== "Unknown") tagsHtml += `<span class="tag tag-size">💾 ${size}</span>`;
            return tagsHtml;
        }
        async function searchMovies() {
            ...
            document.getElementById('status').innerText = "🔍 Scanning Cloud Library...";
            try {
                ...
                data.results.forEach((movie, index) => {
                    ...
                    card.className = 'card'; card.style.animationDelay = `${index * 0.05}s`;
                    const badges = generateTags(movie.title, movie.size);
                    card.innerHTML = `<div class="card-icon"><i class="fa-solid fa-film"></i></div><div class="card-content"><div class="title">${movie.title}</div><div class="meta-tags">${badges}</div></div><button class="download-btn" onclick="getLink(this, '${movie.url}')"><i class="fa-solid fa-cloud-arrow-down"></i> Download</button>`;
        ...
        async function getLink(btn, url) {
            ...
            btn.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> Getting Link...`; btn.style.opacity = "0.7";
            try {
                const res = await fetch(`/api/get-link?url=${encodeURIComponent(url)}`);
                const data = await res.json();
                if(data.success) {
                    btn.innerHTML = `<i class="fa-solid fa-check"></i> Open!`; btn.style.background = "#00f2ff"; btn.style.color = "#000";
                    window.location.href = data.direct_download_link;
                } else {
                    btn.innerHTML = `Failed`; alert("Failed: " + data.error); btn.innerHTML = originalHTML;
                }
